...
describe("XrmWebApiNode-executeMultiple", () => {
  ...
  it("creates records", async () => {
    ...
    const response = await Xrm.WebApi.online.executeMultiple([[account1Create, account1Create], account1Create]);
    ...
    const account3 = getGuidFromHeaders(response[2].headers);

    await Xrm.WebApi.deleteRecord("account", account1);
    await Xrm.WebApi.deleteRecord("account", account2);
    await Xrm.WebApi.deleteRecord("account", account3);
  }, 10000);
