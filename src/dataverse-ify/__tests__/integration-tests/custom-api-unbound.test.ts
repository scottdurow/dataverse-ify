...
describe("XrmContextDataverseClient", () => {
  ...
  it("custom unbound api mapping - using implicit id field", async () => {
    ...
    testRecord1.id = await serviceClient.create(testRecord1);
    try {
      ...
      expect(response.cdsify_UnboundOutEntityReference?.cdsify_integrationtestid).toBe(testRecord1.id);
    } finally {
      await serviceClient.delete(testRecord1);
    }
  }, 100000);

  it("custom unbound api mapping - using explicit id field", async () => {
    // Create an account with sdk types
    setMetadataCache(metadataCache);
    const serviceClient = new XrmContextDataverseClient(Xrm.WebApi);
    // Create test record
    ...
    try {
      ...
      const response = (await serviceClient.execute(request as any)) as cdsify_UnboundEchoResponse;
      assertResponse(response, request);
      expect(response.cdsify_UnboundOutEntityReference?.cdsify_integrationtestid).toBe(
